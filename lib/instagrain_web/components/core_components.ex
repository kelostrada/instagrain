defmodule InstagrainWeb.CoreComponents do
  @moduledoc """
  ...
  attr :color, :atom, values: [:blue, :grey], default: :blue
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg py-1 px-4",
        "text-sm font-semibold leading-6",
        @class,
        @color == :blue && "bg-sky-500 hover:bg-blue-500 text-white active:text-white/80",
        @color == :grey && "bg-neutral-200/70 hover:bg-neutral-300 text-black active:text-black/80"
      ]}
      {@rest}
    >
      <%= render_slot(@inner_block) %>
  ...
  def input(%{field: %Phoenix.HTML.FormField{} = field} = assigns) do
    ...
    |> assign_new(:name, fn ->
      if assigns.multiple, do: field.name <> "[#{assigns.index}]", else: field.name
    end)
  ...
  def input(assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        value={
          if @multiple,
            do: Phoenix.HTML.Form.normalize_value(@type, @value[to_string(@index)]),
            else: Phoenix.HTML.Form.normalize_value(@type, @value)
        }
        class={
          if @class,
            do: @class,
  ...
  def icon(%{name: "instagrain-logo"} = assigns) do
    ~H"""
    <svg
      aria-label="Instagrain"
      class={@class}
      fill="currentColor"
      height="24"
      role="img"
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(InstagrainWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(InstagrainWeb.Gettext, "errors", msg, opts)
    end
  end
